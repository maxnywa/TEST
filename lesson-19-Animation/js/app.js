...
function findElement(index) {
    return document.querySelector(`[data-index = "${index}"]`).closest('.card')
}
...
function onAddToFavorite(e) {
    if(e.target.classList.contains('add-favorite')){
        ...
        news.addFavoriteNews(oneNews)
            .then(alert.showAlert({error:false,text:'News added to Favorite News'}))
            .catch(err => console.log(err))
